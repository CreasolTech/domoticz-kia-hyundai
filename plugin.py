...
#
"""
<plugin key="bluvo" name="Kia UVO and Hyundai Bluelink" author="Pierre Levres" version="0.1.1">
    <description>
        <h2>BlUvo Plugin</h2>
        A plugin for Kia UVO and Hyundai Bluelink EV's (generally MY2020 and beyond). Use at own risk!
        More info at <a href="https://www.domoticz.com/wiki/Vehicles_Hyundai_Kia">Domoticz wiki</a>
        <br/>
...
class BasePlugin:
    ...
    def onStart(self):
        ...
        '''
        if (len(Devices) < 13):
            Domoticz.Device(Unit=1, Type=113, Subtype=0 , Switchtype=3 , Name="km total").Create()
            ...
            Domoticz.Device(Unit=4, TypeName="Percentage"              , Name="Battery charge").Create()
            Domoticz.Device(Unit=5, TypeName="Percentage"              , Name="12v battery charge").Create()
            Domoticz.Device(Unit=6, Type=243, Subtype=31, Switchtype=0 , Name="Status 12v").Create()
            Domoticz.Device(Unit=7, Type=244, Subtype=73, Switchtype=11, Name="Trunk").Create()
            Domoticz.Device(Unit=8, Type=243, Subtype=31, Switchtype=0 , Name="km from home").Create()
            ...
            Domoticz.Device(Unit=12, Type=243, Subtype=22,              Name="Tire pressure alert").Create()
            Domoticz.Device(Unit=13, Type=243, Subtype=31,              Name="Speed").Create()
        ...
        try:
            ...
            if lastHeartbeatTime == 0 or float((datetime.now() - lastHeartbeatTime).total_seconds()) > (random.uniform(0.75,1.5)*(heartbeatmultiplier * heartbeatinterval)):
                ...
                if updated:
                    ...
                    UpdateDevice(12, parsedStatus['tirewarningall'], parsedStatus['tirewarningall'])  #tire pressure alert
                    UpdateDevice(13, parsedStatus['speed'], parsedStatus['speed'])  #speed
        except:
